...
---
### 프로젝트 설명
Github action과 AWS 서비스를 사용한 CI/CD 파이프라인 실습 프로젝트입니다. 
...
2. Github action에서 테스트와 빌드를 수행할 수 있는 환경 설정
3. 테스트가 성공적으로 수행되면 빌드를 수행하고 .zip 형태로 압축
4. AWS S3 접근 권한과 관련된 설정 정보와 AWS CLI를 사용하여 압축된 파일을 S3에 업로드
5. AWS CodePipeLine에서 S3의 배포 프로젝트의 변화를 감지
6. Build 과정은 건너뛰고 Deploy 과정을 수행한다.
7. CodeDeploy에서 지정된 EC2에 빌드된 프로젝트가 반영된다.
